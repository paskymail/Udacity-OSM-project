...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for child in element:
            if child.tag == "tag" and not re.match(PROBLEMCHARS, child.attrib['k']):
                idtag= node_attribs["id"]
                lc = re.match(LOWER_COLON, child.attrib['k'])
                if lc:
                    tipo, key = re.split('[:]', child.attrib["k"], maxsplit=1)
                ...
                tags.append({"id": idtag, "key":key, "value": value, "type": tipo})
        ...
        for child in element:
            ...
            if child.tag == "tag" and not re.match(PROBLEMCHARS, child.attrib['k']):
                ...
                value =child.attrib["v"]
                tags.append({"id": idtag, "key":key, 'type': tipo , "value": value})
            
            elif child.tag == "nd":
                idtag= way_attribs["id"]
                nodeid = child.attrib["ref"]
                way_nodes.append({"id": idtag, 'node_id':nodeid, "position": pos})
                pos = pos +1
...
def validate_element(element, validator, schema=SCHEMA):
    ...
    if validator.validate(element, schema) is not True:
        ...
        field, errors = next(validator._errors.iteritems())
